...
matrix(c(1,1,2,3,5,8), nrow=2, ncol=3, byrow=TRUE)

# R data types
# character
welcomeString <- "Welcome to R with Ameet and Jay"
class(welcomeString)
...
# punctuation with logical operators
# & (and) | (or) ! (negation) != not equal
c = TRUE
d = FALSE
c & d
c | d
...
# vector - a sequence of components of the same type
fruitVector <- c("apple", "orange", "banana", "4")
class(fruitVector)
fruitVector[2]

numericVector <- c(1,1,2,3,5,8,13)
class(numericVector)

# list - a generic vector containing other objects
sampleList <- list(fruitVector,numericVector)
sampleList[[2]][1]
sampleList[[2]][1] <- 5
sampleList[[2]][1]

# matrix - a two dimensional layout 
sampleMatrix <- matrix(c(1,1,2,3,5,8), nrow=2, ncol=3, byrow=TRUE)
sampleMatrix

# Before we move on, lets clear the Global Envionment 
# of objects that we will no longer use
# or you can use the broom icon
rm(list=ls())
...
# An alterantive approach that I prefer:
...
decisionTreeBinary <- rpart(survived ~ . , data = ptitanic, cp=0.012)
